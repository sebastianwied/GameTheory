...
static std::mt19937_64 global_rng((unsigned)chrono::high_resolution_clock::now().time_since_epoch().count());
...
static vector<vector<int>> payoffMatrix = {{1,5},{0,4}};
...
TorusResult torusTournament(AgentGrid agentGrid, int iters, int rounds) {
    int yLen = (int)agentGrid.size();
    int xLen = (int)agentGrid[0].size();
    int N = yLen * xLen;
    TorusResult out;
    vector<vector<double>> totalScore(yLen, vector<double>(xLen, 0.0));
    int snapEvery = max(1, rounds / 100);
    ...
    for (int round=0; round<rounds; ++round) {
        ...
        if (round == 1 || ((round % snapEvery) == 0 && (round / snapEvery) > 0)) {
            ...
            cout << "progress: " << (round / snapEvery) << " / 100\n";
        }
    } // end rounds
...
TorusResult torusTournamentNoEvolution(AgentGrid agentGrid, int iters, int rounds) {
    int yLen = (int)agentGrid.size();
    int xLen = (int)agentGrid[0].size();
    int N = yLen * xLen;
    TorusResult out;
...
int main(int argc, char** argv) {
    ...
    TorusResult resu = torusTournament(grid, iters, rounds);
