...
def displayMemoryOne(agents, agentFrames, agentRuleFrames, scoreFrames):
    ...
    for i, val in enumerate(indexarr):
        if (val[0] > prev) and val[0] < len(colorlist) : 
            ax1.text(frame0.shape[1], i + (frame0.shape[0]//(2*len(colorlist))), f'{agentIDs[val[0]]}', ha="center", va="center", color="k", rotation=-20)
        prev = val[0]
    agim = ax4.hist(np.ravel(scoreFrames[0]))
    ax4.set_title("Score distribution")
    ax4.set_xlim(0, np.nanmax(scoreFrames[-1])/len(scoreFrames))
    def scoreHistUpdate(frame):
        ax4.cla()
        ax4.hist(np.ravel(scoreFrames[frame]))
        ax4.set_title("Score distribution")
        ax4.set_xlim(0, payoff[0]*iters)
    aniag = anim.FuncAnimation(fig=fig1, func=scoreHistUpdate, frames=len(scoreFrames), interval=framerate)
